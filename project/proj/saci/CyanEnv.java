 ...
 *
 */
public class CyanEnv {


//# add a new boolean: creatingEvalContextObject. Here 'self' is replaced by 'newSelf__'
	/**
	...
	public CyanEnv(ProgramUnit genericProgramUnit, ExprGenericPrototypeInstantiation exprGenericType, Env env,
		...
		if ( isGPWithVaryingNumberParam ) {
			for ( ArrayList<Expr> exprList : exprGenericType.getRealTypeListList() ) {
				ArrayList<String> realParamList = new ArrayList<>();
				for ( Expr e : exprList ) {
					if ( e instanceof ExprGenericPrototypeInstantiation ) {
						...
						Tuple<String, Type> t = genProtoInst.ifPrototypeReturnsNameWithPackageAndType(env);
						if ( t == null || t.f2 == null ) {
			...
			i = 0;
			for ( ArrayList<GenericParameter> gpList : genericProgramUnit.getGenericParameterListList()  ) {
				j = 0;
				ArrayList<String> realParamList = new ArrayList<>();
	...
	private static String extractGenericParam(Env env, String realParam, Expr e) {
		...
		if ( Character.isUpperCase(realParam.charAt(0)) ) {
			if (  realParam.indexOf('.') >= 0  ) { 
				...
				if ( realParam.equals(NameServer.dynName) )
					return realParam;
	...
	public ArrayList<ArrayList<String>> getRealParamListList() {
	 ...
	 */
	private ArrayList<ArrayList<String>> realParamListList;
