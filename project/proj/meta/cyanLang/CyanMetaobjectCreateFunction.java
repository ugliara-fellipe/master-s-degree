...
public class CyanMetaobjectCreateFunction extends CyanMetaobjectWithAt 
	...
	public StringBuffer dpa_codeToAdd(ICompilerAction_dpa compiler) {
		...
		*/
		...
		if ( compiler.getCurrentPrototypeName().startsWith("UFunction") ) {
			...
			for ( ArrayList<String> sList : strListList ) { 
				s.append("<");
		...
		*/
		...
		if ( strListList.size() < 1 ) {
			...
			for ( ArrayList<String> strList  : strListList ) {
				...
				if ( count == sizeListList - 1 ) {
					...
					for (int i = 0; i < size - 1; ++i) {
						...
						if ( typeName.equals(NameServer.noneArgumentNameForFunctions) ) {
							if ( size != 2 ) {
								compiler.error(this.getMetaobjectAnnotation().getFirstSymbol(), 
										"The '" + NameServer.noneArgumentNameForFunctions + "' argument for a function type should be alone between "
			...
			if ( sizeListList == 1 ) {
				...
				if ( size != 2 || ! strList.get(0).equals(NameServer.noneArgumentNameForFunctions) ) {
					/*
