	...
	public void genCyanReplacingGenericParameters(PWInterface pw, boolean printInMoreThanOneLine, CyanEnv cyanEnv, boolean genFunctions) {
		if ( cyanEnv.getCreatingInstanceGenericPrototype() ) {
			...
			if ( value == null )
				...
				pw.print("\"" + NameServer.removeQuotes(value) + "\"");
				// pw.print(value);
			}
			/*
			char []charArray = CyanMetaobjectWithAtAnnotation.replaceOnly(name.toCharArray(), cyanEnv.getFormalParamToRealParamTable(), ReplacementPolicyInGenericInstantiation.REPLACE_BY_CYAN_VALUE);
			if ( charArray[0] != '\"' ) 
				pw.print("\"");
			pw.print(charArray);
			if ( charArray[0] != '\"' ) 
